/** @file API.h
 * @brief Provides the high-level user functionality intended for use by typical
 * VEX Cortex
 * programmers.
 *
 * This file should be included for you in the predefined stubs in each new VEX
 * Cortex PROS
 * project through the inclusion of "main.h". In any new C source file, it is
 * advisable to
 * include main.h instead of referencing API.h by name, to better handle any
 * nomenclature
 ...
 * is pressed, or
 * false otherwise. If no joystick is connected to the requested slot, returns
 * false.
 *
 * @param joystick the joystick slot to check
 * @param buttonGroup one of 5, 6, 7, or 8 to request that button as labelled on
 * the joystick
 ...
 *
 * In rare circumstances, this method might return 0. Check the output value for
 * reasonability
 ...
 * communication.
 *
 * This excludes the crystal ports but includes the Communications, Speaker, and
 * Analog ports.
 *
 * The motor ports are not on the Cortex and are thus excluded from this count.
 * Pin 0 is the
 * Speaker port, pins 1-12 are the standard Digital I/O, 13-20 are the Analog
 * I/O, 21+22 are
 ...
 * input to read as
 * "HIGH" when unplugged, but is fairly weak and can safely be driven by most
 * sources. Many VEX
 ...
 *
 * This is the default state for the 8 Analog pins and the Speaker port. This
 * only works on
 * pins with analog input capabilities; use anywhere else results in undefined
 * behavior.
 */
 ...
 *
 * Beware of power consumption, as digital inputs left "floating" may switch
 * back and forth
 ...
 *
 * This is the mode which should be used to output a digital HIGH or LOW value
 * from the Cortex.
 ...
 *
 * This is useful in a few cases for external electronics and should not be used
 * for the VEX
 * solenoid or LEDs.
 */
 ...
 * this time and
 * computes an average from approximately 500 samples, 1 ms apart, for a 0.5 s
 * period of
 * calibration. The average value thus calculated is returned and stored for
 * later calls to the
 * analogReadCalibrated() and analogReadCalibratedHR() functions. These
 * functions will return
 * the difference between this value and the current sensor value when called.
 *
 * Do not use this function in initializeIO(), or when the sensor value might be
 * unstable
 * (gyro rotation, accelerometer movement).
 *
 * This function may not work properly if the VEX Cortex is tethered to a PC
 * using the orange
 * USB A to A cable and has no VEX 7.2V Battery connected and powered on, as the
 * VEX Battery
 ...
 *
 * The analogCalibrate() function must be run first on that channel. This
 * function is
 ...
 * precision.
 *
 * The analogCalibrate() function must be run first. This is intended for
 * integrated sensor
 * values such as gyros and accelerometers to reduce drift due to round-off, and
 * should not be
 ...
 * only reads
 * 12 bits, so that errors induced by the average value being between two values
 * come out
 * in the wash when integrated over time. Think of the value as the true value
 * times 16.
 *
 * This function may not work properly if the VEX Cortex is tethered to a PC
 * using the orange
 ...
 *
 * If the pin is configured as some other mode, the digital value which reflects
 * the current
 * state of the pin is returned, which may or may not differ from the currently
 * set value. The
 * return value is undefined for pins configured as Analog inputs, or for ports
 * in use by a
 ...
 * @param value an expression evaluating to "true" or "false" to set the output
 * to HIGH or LOW
 ...
 * sensors are
 * open-drain active low. It should not be a big deal for most push-pull
...
/*
 * Digital port 10 cannot be used as an interrupt port, or for an encoder. Plan
 * accordingly.
 */

/**
 * When used in ioSetInterrupt(), triggers an interrupt on rising edges (LOW to
 * HIGH).
 */
#define INTERRUPT_EDGE_RISING 1
/**
 * When used in ioSetInterrupt(), triggers an interrupt on falling edges (HIGH
 * to LOW).
 */
#define INTERRUPT_EDGE_FALLING 2
/**
 * When used in ioSetInterrupt(), triggers an interrupt on both rising and
 * falling edges
 * (LOW to HIGH or HIGH to LOW).
 */
#define INTERRUPT_EDGE_BOTH 3
/**
 * Type definition for interrupt handlers. Such functions must accept one
 * argument indicating
...
/**
 * Sets up an interrupt to occur on the specified pin, and resets any counters
 * or timers
 * associated with the pin.
 *
 * Each time the specified change occurs, the function pointer passed in will be
 * called with
 * the pin that changed as an argument. Enabling pin-change interrupts consumes
 * processing
 * time, so it is best to only enable necessary interrupts and to keep the
 * InterruptHandler
 * function short. Pin change interrupts can only be enabled on pins 1-9 and
 * 11-12.
 *
 * Do not use API functions such as delay() inside the handler function, as the
 * function will
 * run in an ISR where the scheduler is paused and no other interrupts can
 * execute. It is best
 * to quickly update some state and allow a task to perform the work.
 *
 * Do not use this function on pins that are also being used by the built-in
 * ultrasonic or
 ...
 * @param edges one of INTERRUPT_EDGE_RISING, INTERRUPT_EDGE_FALLING, or
 * INTERRUPT_EDGE_BOTH
 * @param handler the function to call when the condition is satisfied
 */
void ioSetInterrupt(unsigned char pin, unsigned char edges,
 ...
 *
 * This speed may have been set by any task or the PROS kernel itself. This is
 * not guaranteed
 ...
 * @return the speed last sent to this channel; -127 is full reverse and 127 is
 * full forward,
 * with 0 being off
 */
 ...
 *
 * Do not use motorSet() with the same channel argument from two different
 * tasks. It is safe to
 * use motorSet() with different channel arguments from different tasks.
 *
 * @param channel the motor channel to modify from 1-10
 * @param speed the new signed speed; -127 is full reverse and 127 is full
 * forward, with 0
 * being off
 */
void motorSet(unsigned char channel, int speed);
/**
 * Stops the motor on the specified channel, equivalent to calling motorSet()
 * with an argument
 * of zero.
 *
...
/**
 * Stops all motors; significantly faster than looping through all motor ports
 * and calling
 ...
 *
 * The VEX speaker is not thread safe; it can only be used from one task at a
 * time. Using the
 * VEX speaker may impact robot performance. Teams may benefit from an if
 * statement that only
 ...
 * VEX speaker. The audio is mixed to allow polyphonic sound to be played. Many
 * simple songs
 * are available in RTTTL format online, or compose your own.
 *
 * The song must not be NULL, but unused tracks within the song can be set to
 * NULL. If any of
 ...
 *
 * The song must not be NULL. If an invalid song is specified, the result of
 * this function is
 * undefined.
 *
 * The VEX speaker is not thread safe; it can only be used from one task at a
 ...
 *
 * If a song is currently being played in another task, the behavior of this
 * function is
...
/**
 * IME addresses end at 0x1F. Actually using more than 10 (address 0x1A)
 * encoders will cause
 ...
 *
 * IMEs are assigned sequential incrementing addresses, beginning with the first
 * IME on the
 * chain (closest to the VEX Cortex I2C port). Therefore, a given configuration
 * of IMEs will
 * always have the same ID assigned to each encoder. The addresses range from 0
 * to
 * IME_ADDR_MAX, so the first encoder gets 0, the second gets 1, ...
 *
 * This function should most likely be used in initialize(). Do not use it in
 * initializeIO() or
 * at any other time when the scheduler is paused (like an interrupt). Checking
 * the return
 * value of this function is important to ensure that all IMEs are plugged in
 * and responding as
 * expected.
 *
 * This function, unlike the other IME functions, is not thread safe. If using
 * imeInitializeAll
 * to re-initialize encoders, calls to other IME functions might behave
 ...
 *
 * Much like the count for a quadrature encoder, the tick count is signed and
 * cumulative.
 * The value reflects total counts since the last reset. Different VEX Motor
 * Encoders have a
 ...
 *
 * If the IME address is invalid, or the IME has not been reset or initialized,
 * the value
 * stored in *value is undefined.
 *
 * @param address the IME address to fetch from 0 to IME_ADDR_MAX
 * @param value a pointer to the location where the value will be stored
 * (obtained using the
 * "&" operator on the target variable name e.g. <code>imeGet(2,
 * &counts)</code>)
 * @return true if the count was successfully read and the value stored in
 * *value is valid;
 * false otherwise
 */
 ...
 * increasing and
 * negative if the IME count is decreasing. The velocity is in RPM of the
 * internal encoder
 ...
 * @param value a pointer to the location where the value will be stored
 * (obtained using the
 * "&" operator on the target variable name e.g. <code>imeGetVelocity(2,
 * &counts)</code>)
 * @return true if the velocity was successfully read and the value stored in
 * *value is valid;
 * false otherwise
 */
...
/**
 * Shuts down all IMEs on the chain; their addresses return to the default and
 * the stored
 * counts and velocities are lost. This function, unlike the other IME
 * functions, is not
 * thread safe.
 *
 * To use the IME chain again, wait at least 0.25 seconds before using
 * imeInitializeAll again.
 */
void imeShutdown();
 ...
 *
 * NULL will be returned if the port is invalid or the gyro is already in use.
 * Initializing a
 * gyro implicitly calibrates it and resets its count. Do not move the robot
 * while the gyro is
 ...
 * default value
 * at this time is 196; higher values will increase the number of degrees
 * reported for a fixed
 * actual rotation, while lower values will decrease the number of degrees
 * reported. If your
 * robot is consistently turning too far, increase the multiplier, and if it is
 * not turning
 ...
 *
 * It is safe to use this method while a gyro is enabled. It is not necessary to
 * call this
 ...
 *
 * Neither the top port nor the bottom port can be digital port 10. NULL will be
 * returned if
 * either port is invalid or the encoder is already in use. Initializing an
 * encoder implicitly
 * resets its count.
 *
 * @param portTop the "top" wire from the encoder sensor with the removable
 * cover side UP
 ...
 * @return an Encoder object to be stored and used for later calls to encoder
 * functions
 */
 ...
 *
 * Encoders use processing power, so disabling unused encoders increases code
 * performance.
 ...
 *
 * Ultrasonic information is stored as an opaque pointer to a structure in
 * memory; as this is a
 * pointer type, it can be safely passed or stored by value.
 */
 ...
 * started, the
 * return value is undefined. Round and fluffy objects can cause inaccurate
 * values to be
 * returned.
 *
 ...
 *
 * The ultrasonic sensor will be polled in the background in concert with the
 * other sensors
 * registered using this method. NULL will be returned if either port is invalid
 * or the
 ...
 * @return an Ultrasonic object to be stored and used for later calls to
...
// ---- I2C port control ----
/**
 * i2cRead - Reads the specified number of data bytes from the specified 7-bit
 * I2C address. The
 * bytes will be stored at the specified location. Returns true if successful or
 * false if
 ...
 * supplied.
 *
 * Since most I2C devices use an 8-bit register architecture, this method has
 * limited
 ...
 * address on
 * the specified 7-bit I2C address. Returns true if successful or false if
 * failed. If only some
 ...
 * supplied.
 *
 * Most I2C devices support an auto-increment address feature, so using this
 * method to read
 * more than one byte will usually read a block of sequential registers. Try to
 * merge reads to
 * separate registers into a larger read using this function whenever possible
 * to improve code
 ...
 * I2C address.
 * Returns true if successful or false if failed. If only smoe bytes could be
 * written, false
 * is still returned.
 *
 * The I2C address should be right-aligned; the R/W bit is automatically
 ...
 * While useful for
 * the vast majority of I2C operations, writing multiple bytes requires the
 * i2cWrite method.
 */
 ...
 *
 * FILE * is the standard library method of referring to a file pointer, even
 * though there is
 ...
 * parameters.
 *
 * I/O to the port is accomplished using the "standard" I/O functions such as
 * fputs(),
 * fprintf(), and fputc().
 *
 * Re-initializing an open port may cause loss of data in the buffers. This
 * routine may be
 * safely called from initializeIO() or when the scheduler is paused. If I/O is
 * attempted on a
 * serial port which has never been opened, the behavior will be the same as if
 * the port had
 * been disabled.
 *
 * @param usart the port to open, either "uart1" or "uart2"
 * @param baud the baud rate to use from 2400 to 1000000 baud
 * @param flags a bit mask combination of the SERIAL_* flags specifying parity,
 * stop, and data
 * bits
 */
 ...
 *
 * Any data in the transmit and receive buffers will be lost. Attempts to read
 * from the port
 * when it is disabled will deadlock, and attempts to write to it may deadlock
 * depending on
...
/**
 * Closes the specified file descriptor. This function does not work on
 * communication ports;
 ...
 * number of
 * characters available) from the specified stream. This only works for
 * communication ports and
 * files in Read mode; for files in Write mode, 0 is always returned.
 *
 * This function may underestimate, but will not overestimate, the number of
 * characters which
 * meet this criterion.
 *
 * @param stream the stream to read (stdin, uart1, uart2, or an open file in
 * Read mode)
 * @return the number of characters which meet this criterion; if this number
 * cannot be
 ...
 *
 * The file will actually be erased from memory on the next re-boot. A physical
 * power cycle is
 * required to purge deleted files and free their allocated space for new files
 * to be written.
 ...
 *
 * @param stream the channel to check (stdin, uart1, uart2, or an open file in
 * Read mode)
 ...
 * function has no
 * effect on a communication port or a file in Read mode, as these streams are
 * always flushed as
 * quickly as possible by the kernel.
 *
 * Successful completion of an fflush function on a file in Write mode cannot
 * guarantee that
 ...
 * memory at str.
 * Characters will be read until the specified limit is reached, a new line is
 * found, or the
 * end of file is reached.
 *
 * If the stream is already at end of file (for files in Read mode), NULL will
 * be returned;
 ...
 * @param num the maximum number of characters to store; at most (num - 1)
 * characters will be
 * read, with a null terminator ('\0') automatically appended
 * @param stream the channel to read (stdin, uart1, uart2, or an open file in
 * Read mode)
 ...
 * eight characters.
 * Only four files can be in use simultaneously in any given time, with at most
 * one of those
 * files in Write mode. This function does not work on communication ports; use
 * usartInit()
 * instead.
 *
 * mode can be "r" or "w". Due to the nature of the VEX Cortex memory, the "r+",
 * "w+", and "a"
 * modes are not supported by the file system.
 *
 * Opening a file that does not exist in Read mode will fail and return NULL,
 * but opening a new
 * file in Write mode will create it if there is space. Opening a file that
 * already exists in
 * Write mode will destroy the contents and create a new blank file if space is
 * available.
 *
 * There are important considerations when using of the file system on the VEX
 * Cortex. Reading
 * from files is safe, but writing to files should only be performed when robot
 * actuators have
 * been stopped. PROS will attempt to continue to handle events during file
 * writes, but most
 * user tasks cannot execute during file writing. Powering down the VEX Cortex
 * mid-write may
 ...
 *
 * This method is much, much faster than fprintf() and does not add a new line
 * like fputs().
 ...
 *
 * If the memory at ptr cannot store (size * count) bytes, undefined behavior
 * occurs.
 *
 ...
 * @param stream the stream to read (stdout, uart1, uart2, or an open file in
 * Read mode)
 ...
 * @param origin the reference location for offset: SEEK_CUR, SEEK_SET, or
 * SEEK_END
 ...
 * value.
 *
 * When using a wireless connection, one may need to press the spacebar before
 * the input is
 ...
 * undefined behavior.
 * Other characters are written out verbatim. Do not use fprintf() on a VEX LCD
 * port.
 ...
 * character, so an
 * argument of 256 will cause a maximum of 255 non-null characters to be
 * printed, and one null
 ...
 *
 * If the buffer is not big enough to contain the complete formatted output,
 * undefined behavior
 ...
 *
 * Printing to a line implicitly overwrites the contents, so clearing should
 * only be required
 * at startup.
 *
 ...
 *
 * If the LCD was not initialized before, the text currently on the screen will
 * be undefined.
 * The port will not be usable with standard serial port functions until the LCD
 * is stopped.
 *
 ...
 *
 * The output string will be truncated as necessary to fit on the LCD screen, 16
 * characters
 * wide. It is probably better to generate the string in a local buffer and use
 * lcdSetText()
...
lcdPrint(FILE *lcdPort, unsigned char line, const char *formatString, ...);
#endif
/**
 * Reads the user button status from the LCD display.
 *
 * For example, if the left and right buttons are pushed, (1 | 4) = 5 will be
 * returned. 0 is
 ...
 *
 * Turning it off will save power but may make it more difficult to read in dim
 * conditions.
 *
 ...
 * The output string will be truncated as necessary to fit on the LCD screen, 16
 * characters
 * wide. This function, like fprint(), is much, much faster than a formatted
 * routine such as
...
/**
 * Only this many tasks can exist at once. Attempts to create further tasks will
 * not succeed
 * until tasks end or are destroyed, AND the idle task cleans them up.
 *
 * Changing this value will not change the limit without a kernel recompile. The
 * idle task
 * and VEX daemon task count against the limit. The user autonomous() or
 * teleop() also counts
...
/**
 * The lowest priority that can be assigned to a task, which puts it on a level
 * with the idle
 * task. This may cause severe performance problems and is generally not
 * recommended.
 */
...
/**
 * The highest priority that can be assigned to a task. Unlike the lowest
 * priority, this
 ...
 *
 * This is probably OK for 4-5 levels of function calls and the use of printf()
 * with several
 * arguments. Tasks requiring deep recursion or large local buffers will need a
 * bigger stack.
 */
#define TASK_DEFAULT_STACK_SIZE 512
/**
 * The minimum stack depth for a task. Scheduler state is stored on the stack,
 * so even if the
 * task never uses the stack, at least this much space must be allocated.
 *
 * Function calls and other seemingly innocent constructs may place information
 * on the stack.
 ...
 * @param stackDepth the number of variables available on the stack (4 *
 * stackDepth bytes will
 * be allocated on the Cortex)
 * @param parameters an argument passed to the taskCode function
 * @param priority a value from TASK_PRIORITY_LOWEST to TASK_PRIORITY_HIGHEST
 * determining the
 ...
 *
 * Delaying for a period of zero will force a reschedule, where tasks of equal
 * priority may be
 * scheduled if available. The calling task will still be available for
 * immediate rescheduling
 * once the other tasks have had their turn or if nothing of equal or higher
 * priority is
 * available to be scheduled.
 *
 * This is not the best method to have a task execute code at predefined
 * intervals, as the
 * delay time is measured from when the delay is requested. To delay cyclically,
 * use
 * taskDelayUntil().
 *
 * @param msToDelay the number of milliseconds to wait, with 1000 milliseconds
 * per second
 */
void taskDelay(const unsigned long msToDelay);
/**
 * Delays the current task until a specified time. The task will be unblocked
 * at the time *previousWakeTime + cycleTime, and *previousWakeTime will be
 * changed to reflect
 ...
 *
 * This function should be used by cyclical tasks to ensure a constant execution
 * frequency.
 * While taskDelay() specifies a wake time relative to the time at which the
 * function is
 * called, taskDelayUntil() specifies the absolute future time at which it
 * wishes to unblock.
 * Calling taskDelayUntil with the same cycleTime parameter value in a loop,
 * with
 * previousWakeTime referring to a local variable initialized to millis(), will
 * cause the
 * loop to execute with a fixed period.
 *
 * @param previousWakeTime a pointer to the location storing the last unblock
 * time, obtained
 ...
 *
 * Deleting the last task will end the program, possibly leading to undesirable
 * states as
 * some outputs may remain in their last set configuration.
 *
 * The idle task is responsible for freeing the kernel allocated memory from
 * tasks that
 * have been deleted. It is therefore important that the idle task is not
 * starved of
 * processing time. Memory allocated by the task code is not automatically
 * freed, and should be
 ...
 *
 * This includes all ready, blocked and suspended tasks. A task that has been
 * deleted but not
 * yet freed by the idle task will also be included in the count. Tasks recently
 * created may
 ...
 *
 * A context switch may occur before the function returns if the priority being
 * set is higher
 * than the currently executing task and the task being mutated is available to
 * be scheduled.
 *
 * @param task the task to change; passing NULL changes the current task
 * @param newPriority a value between TASK_PRIORITY_LOWEST and
 ...
 *
 * A task that has been suspended by one or more calls to taskSuspend() will be
 * made available
 * for scheduling again by a call to taskResume(). If the task was not suspended
 * at the time
 * of the call to taskResume(), undefined behavior occurs.
 *
 * @param taskToResume the task to change; passing NULL is not allowed as the
 * current task
 ...
 * priority than the
 * "main" tasks. The created task will have priority TASK_PRIORITY_DEFAULT + 1
 * with the default
 * stack size. To customize behavior, create a task manually with the specified
 * function.
 *
 * This task will automatically terminate after one further function invocation
 * when the robot
 * is disabled or when the robot mode is switched.
 *
 * @param fn the function to call in this loop
 * @param increment the delay between successive calls in milliseconds; the
 * taskDelayUntil()
 ...
 *
 * When suspended a task will not be scheduled, regardless of whether it might
 * be otherwise
 * available to run.
 *
 ...
 * semaphoreTake() and
 * semaphoreGive() functions. The mutex functions must not be used on objects of
 * this type.
 *
 * This type of object does not need to have balanced take and give calls, so
 * priority
...
/**
 * Signals a semaphore. Tasks waiting for a signal using semaphoreTake() will be
 * unblocked by
 * this call and can continue execution.
 *
 * Slow processes can give semaphores when ready, and fast processes waiting to
 * take the
 * semaphore will continue at that point.
 *
 * @param semaphore the semaphore to signal
 * @return true if the semaphore was successfully given, or false if the
 * semaphore was not
 ...
 * current task
 * will wait for the semaphore to be signaled. Other tasks can run during this
 * time.
 *
 * @param semaphore the semaphore to wait
 * @param blockTime the maximum time to wait for the semaphore to be given,
 * where -1
 ...
 *
 * Mutexes created using this function can be accessed using the mutexTake() and
 * mutexGive()
 * functions. The semaphore functions must not be used on objects of this type.
 *
 * This type of object uses a priority inheritance mechanism so a task 'taking'
 * a mutex MUST
...
Mutex mutexCreate();
/**
 * Relinquishes a mutex so that other tasks can use the resource it guards. The
 * mutex must be
 ...
 * already
 * holds the mutex, the function will wait for the mutex to be released. Other
 * tasks can run
 * during this time.
 *
 * @param mutex the mutex to request
 * @param blockTime the maximum time to wait for the mutex to be available,
 * where -1
 * specifies an infinite timeout
 ...
 *
 * The method used for delaying this length of time may vary depending on the
 * argument.
 * The current task will always be delayed by at least the specified period, but
 * possibly much
 ...
 * microseconds in a
 * second, so as a 32-bit integer, this will overflow and wrap back to zero
 * every two hours or
 * so.
 *
 * This function is Wiring-compatible.
 *
 * @return the number of microseconds since the Cortex was turned on or the last
 * overflow
 */
unsigned long micros();
/**
 * Returns the number of milliseconds since Cortex power-up. There are 1000
 * milliseconds in a
